...
import kaneki.modules.sql.kuki_sql as sql
from kaneki import dispatcher
from kaneki.modules.helper_funcs.chat_status import user_admin, user_admin_no_reply
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "KanekiAI disable by {}.".format(mention_html(user.id, user.first_name)),
...
def kukiadd(update: Update, context: CallbackContext) -> str:
    ...
    if match:
        ...
        if is_kuki:
            ...
            update.effective_message.edit_text(
                "KanekiAI enable by {}.".format(mention_html(user.id, user.first_name)),
                parse_mode=ParseMode.HTML,
            )
...
def kuki(update: Update, context: CallbackContext):
    ...
    msg = "Choose an option to activated kaneki ai"
...
def ai(update: Update, context: CallbackContext):
    ...
    if message.text and not message.document:
        ...
        kukiurl = requests.get(
            "https://kukiapi.xyz/api/apikey=KUKITnhtN2I89/Kaneki/Cyberhunt27/message="
            + Message
        )
