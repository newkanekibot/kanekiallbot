...
# the file part of kaneki rewriten by @Cyberhunt27
...
from kaneki import BOT_USERNAME, DEV_USERS, pbot
from kaneki.modules.mongo.captcha_mongo import manage_db
from kaneki.utils.markup import MakeCaptchaMarkup
...
async def check_chat_captcha(client, message):
    ...
    if CC_API is None:
        await client.send_message(chat_id, "join @TebBotSupport")
        return
...
async def add_chat(bot, message):
    if CC_API is None:
        await message.reply_text("join @TebBotSupport")
        return
    chat_id = message.chat.id
    user_id = message.from_user.id
