...
from kaneki import JOIN_LOGGER, LOGGER, OWNER_ID, SUPPORT_CHAT, dispatcher
...
def export_data(update, context):
    ...
    with open("kaneki{}.backup".format(chat_id), "w") as f:
    ...
    context.bot.sendDocument(
        current_chat_id,
        document=open("kaneki{}.backup".format(chat_id), "rb"),
        caption="*Successfully Exported backup:*\nChat: `{}`\nChat ID: `{}`\nOn: `{}`\n\nNote: This `kaneki-Backup` was specially made for notes.".format(
            chat.title,
            chat_id,
            tgl,
        ),
    ...
    os.remove("kaneki{}.backup".format(chat_id))  # Cleaning file
